...
class InSegtAnnotator(annotator.Annotator):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key()==PyQt5.QtCore.Qt.Key_I:
            ...
            self.setTitle()
        elif 50 < event.key() < 58:  # Keys 3-9 (ASCII 51-57) - ignore
            self.showInfo('Only labels 1 (fiber) and 2 (background) are available')
        else:
    ...
    def wheelEvent(self, event):
        ...
        # Calculate new source rect centered on mouse position
        # Convert mouse position to image coordinates
        img_x = (mouse_pos.x() - self.padding.x()) / self.zoomFactor + self.source.x()
        img_y = (mouse_pos.y() - self.padding.y()) / self.zoomFactor + self.source.y()

        # Calculate new source size
        new_width = self.source.width() / zoom_factor
        new_height = self.source.height() / zoom_factor

        # Limit zoom range
        min_size = 50  # Minimum zoom (most zoomed in)
        max_width = self.imagePix.width()
        max_height = self.imagePix.height()

        new_width = max(min_size, min(new_width, max_width))
        new_height = max(min_size, min(new_height, max_height))

        # Calculate new top-left, keeping mouse position fixed
        new_x = img_x - (mouse_pos.x() - self.padding.x()) / self.zoomFactor * (new_width / self.source.width())
        new_y = img_y - (mouse_pos.y() - self.padding.y()) / self.zoomFactor * (new_height / self.source.height())

        # Clamp to image bounds
        new_x = max(0, min(new_x, self.imagePix.width() - new_width))
        new_y = max(0, min(new_y, self.imagePix.height() - new_height))

        # Update source rect
        self.source = PyQt5.QtCore.QRect(int(new_x), int(new_y), int(new_width), int(new_height))
